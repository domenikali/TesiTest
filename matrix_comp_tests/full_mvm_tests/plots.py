...
def create_dispersion_plot():
    """
    Reads all .csv files in the current directory, groups them by test case,
    and generates a combined dispersion plot with a 21:9 aspect ratio.
    """
    # --- 1. Discover and load data ---
    data_dir = '.'
    all_files = [f for f in os.listdir(data_dir) if f.endswith('.csv')]
    ...
    # Create a dispersion plot (stripplot)
    sns.stripplot(data=full_df, x='case', y='duration_ns', hue='algorithm', 
                  order=case_order, hue_order=algo_order, ax=ax, 
                  jitter=True, dodge=True, alpha=0.7, size=4)

    # --- 3. Customize the plot ---
    ax.set_title('MVM Algorithm Performance by Test Case (O0 GCC Optimisation)', fontsize=18, pad=20)
    ax.set_xlabel('Test Case', fontsize=14)
    ax.set_ylabel('Execution Time (nanoseconds)', fontsize=14)
    ax.set_yscale('log') # Use a logarithmic scale for better visibility

    # Position legend below the x-axis with a larger font
    ax.legend(title='Algorithm', loc='upper center', bbox_to_anchor=(0.5, -0.15), 
              ncol=len(algo_order), fancybox=True, shadow=True, fontsize=12)
    ...
    # --- 4. Save the plot ---
    output_filename = 'mvm_benchmark_dispersion_plot.png'
    plt.savefig(output_filename, dpi=300, bbox_inches='tight')
    print(f"Dispersion plot saved as '{output_filename}'")
